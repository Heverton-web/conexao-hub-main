      ...
      <div className="rounded-2xl w-full max-w-3xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-slide-up" style={{ backgroundColor: 'var(--color-surface)', border: '1px solid color-mix(in srgb, var(--color-warning) 30%, transparent)' }}>
        <div className="p-6 border-b flex justify-between items-start" style={{ borderColor: 'var(--color-border)', backgroundColor: 'var(--color-surface)' }}>
            <div className="flex gap-4">
                ...
                <div>
                    <h3 className="text-xl font-bold" style={{ color: 'var(--color-text-main)' }}>Configuração do Banco (Modo Aberto)</h3>
        ...
        <div className="p-4 border-t flex justify-between items-center" style={{ backgroundColor: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
            <p className="text-xs" style={{ color: 'var(--color-text-muted)' }}>Após rodar o script, recarregue esta página.</p>
            <div className="flex gap-3">
                <button onClick={() => window.open('https://supabase.com/dashboard/project/_/sql', '_blank')} className="px-4 py-2.5 rounded-lg font-bold transition-colors text-xs uppercase tracking-wide" style={{ backgroundColor: 'var(--color-bg)', color: 'var(--color-text-muted)' }}>
                    Abrir SQL Editor
                </button>
                {onClose ? (
                    <button onClick={onClose} className="px-6 py-2.5 rounded-lg font-bold hover:opacity-90 transition-opacity" style={{ backgroundColor: 'var(--color-text-main)', color: 'var(--color-bg)' }}>
                        Já executei
                    </button>
                ) : (
                    <button onClick={() => window.location.reload()} className="px-6 py-2.5 rounded-lg font-bold hover:opacity-90 transition-opacity text-white" style={{ backgroundColor: 'var(--color-accent)' }}>
